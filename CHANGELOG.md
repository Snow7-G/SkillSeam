...
- frontmatter 按原始行边界提取（修复正则吞掉 `---` 前空行导致的 keep 语义丢失），支持 YAML 块标量（`>` / `|`），折行与收尾规则（clip/strip/keep）符合 YAML 1.2.2 §6.5/§8.1.2，字面标量保留块内额外缩进与纯空白行，手写实现保持零依赖，并有 PyYAML 差分测试背书（144+ 组合）。
