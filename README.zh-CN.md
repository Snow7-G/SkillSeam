...
SkillSeam 把这个选择过程搬到上线之前重演。40 条任务、6 个 skill 的 demo 里，埋进去的 4 处撞车全部现形，且一致率 5/5，干净任务零误报。每条冲突报告都会指出截胡方 description 里惹祸的关键词。
...
```bash
python3 tests/test_atlas.py   # 83 项测试，纯标准库
node tests/web_smoke.cjs      # 26 项网页断言，node >= 18
```
