...
SkillSeam は、出荷前にその選択プロセスを再生します。同梱の 2 つのデモはどちらも 6 スキル・40 タスクの実測で、中国語セットは 4 件、英語セットは 6 件の境界問題を検出しました。いずれも票の一貫性は 5/5 で、揺れた行はありません。各レポートには、横取りの原因となった description 内のキーワードが特定して表示されます。

## クイックスタート（Web、インストール不要）

[https://snow7-g.github.io/SkillSeam/?lang=en](https://snow7-g.github.io/SkillSeam/?lang=en) を開き、デモボタンを押すと 10 秒でヒートマップが表示されます。自分のスキルを貼り付けて、API キーを追加し（キーはブラウザ内のみ、リクエストはプロバイダーへ直通）、「実行」を押すだけです。
